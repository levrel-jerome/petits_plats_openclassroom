export default function recetteFactory(data) {
  ...
  function getRecetteCardDOM() {
    ...
    const timerFont = document.createElement("div");
    timerFont.className = "timer";
    const timer = document.createElement("p");
    const horloge = document.createElement("span");
    horloge.className = "far fa-clock mt-2";
    timer.className = "timer";
    timer.textContent = `${time} min`;

    titleTime.appendChild(timerFont);
    timerFont.appendChild(horloge);
    timerFont.appendChild(timer);
